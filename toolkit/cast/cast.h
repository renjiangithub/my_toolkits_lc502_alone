...
template<class T>
void num_to_bytes_(const T num, uint8_t* bytes) {
    const std::size_t width = sizeof(num);  // size in bytes

    for (int i = 0; i < width; ++i) {
        bytes[i] = static_cast<uint8_t>(num >> (i * 8));
    }
}
...
template<class T>
T bytes_to_num_(const uint8_t* bytes) {
    T result = 0;
    const std::size_t width = sizeof(result);  // size in bytes
...
template<class T>
T bytes_to_num_(const uint8_t* bytes, std::size_t len) {
    T result = 0;
    const std::size_t width = std::min(sizeof(result), len);  // size of bytes
